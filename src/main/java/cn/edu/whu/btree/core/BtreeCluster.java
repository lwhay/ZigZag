...
public class BtreeCluster<T extends Comparable<T>, V> {
    ...
    private int nodeNumOfBlock, cacheSize, nodecachesize, inodensize = 12, internsize, leafnsize, interInitial = -1,
            leafInitial = -2, InterNum, LeafNum, InterBlockNum, LeafBlockNum, keylen, valuelen, rootaddr, keyNum,
            blockSize, headaddr, tailaddr,
            size;
    ...
    public BtreeCluster(int nodeNumOfBlock, String path, int cacheSize, int blocksize, float cachefac) {
        ...
        this.readNo = this.writeNo = this.InterNum = this.LeafNum = this.InterBlockNum = this.LeafBlockNum = 0;
        this.type = false;
    ...
    private void loadNodeRestrict(iNode node) {
        ...
        if (cache.getNode(((InternalNode) node).pointers[0]) instanceof BtreeCluster.LeafNode) {
            return;
        }
    ...
    public BtreeCluster(List<BtreeCluster<T, V>> btreeList, int nodeNumOfBlock, String path, int cacheSize,
            int blocksize, float cachefac) {
        this(nodeNumOfBlock, path, cacheSize, blocksize, cachefac);
    ...
    private class InnerIterator implements Iterator<IteratorEntry<T, V>> {
        ...
        public IteratorEntry<T, V> next() {
            ...
            return new IteratorEntry<T, V>(key, value);
        }
    ...
    class InternalNode extends iNode {
        ...
        @SuppressWarnings("unchecked")
        public iNode insert(T key, V value, TreeCache cache) {
            int pos = -1;
        ...
        public V find(T key, TreeCache cache) {
            ...
            //                    break;
            //            }
            //            System.out.println(pos + " : " + i + " " + this.keys[pos] + " : " + this.keys[i] + " " + key);
    ...
    private class LeafNode extends iNode {
        ...
        public V find(T key, TreeCache cache) {
            ...
            int middle = Utils.bSearchBasicLeaf(this.keys, 0, this.size, key);
            if (middle >= this.size) {
                return null;
            }
    ...
    class TreeCache {
        ...
        /*
         * key����洢����Ϊ�����Ǹ���hash���key�����ǲ���ɾ���Ȳ�����Ҫ����������������hash����Ȼ�޷���֤lru������lruԭ�����
         * ����ɾ��valueʱֻ�Ǹ��������ڲ���������ɾ��������һ��key����������hash��ɾ��
         */
                    ...
                    for (Iterator<Integer> it = temp.keySet().iterator(); it.hasNext();) {
                        ...
                        if (block == null) {
                            if (blockFlag.get(bno) && nodeNumOfBlock != 1) {
                                ...
                                int type = tempNode instanceof BtreeCluster.LeafNode ? 2 : 1;
    ...
    public static class IteratorEntry<T extends Comparable<T>, V> implements Comparable<IteratorEntry<T, V>> {
        private T key;
        private V value;
        ...
        @Override
        public int compareTo(IteratorEntry<T, V> o) {
            // TODO Auto-generated method stub
