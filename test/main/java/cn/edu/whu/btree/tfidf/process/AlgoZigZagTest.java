...
import cn.edu.whu.tfidf.process.TfidfPref;
import cn.edu.whu.tfidf.storage.index.TfidfPrefIndex;

public class AlgoZigZagTest {
	...
	public static void main(String[] args) {
		...
		System.out.println("ZigZag token:" + index.getTokenBtree().getCacheSize() + "  reverse:"
		...
		iSelect select = new TfidfPref(index, qExtendedNum, rExtendedNum, useLengthIndex);
		List<List<Integer>> queryList = new ArrayList<List<Integer>>();
		BufferedReader queryReader;
		try {
			queryReader = new BufferedReader(new InputStreamReader(new FileInputStream(new File(queryPath)), "utf-8"));
			String line = null;
		...
		System.out.println("threshold=" + threshold + " rExtendedNum=" + rExtendedNum + " findTimes=" + findTimes);
		...
		System.out.println("candidate : " + ((TfidfPref) select).getCandiSize() * 1.0 / findTimes);

		System.out.println("prefix : " + ((TfidfPref) select).getPrefixTokens() * 1.0 / findTimes + " : "
				+ ((TfidfPref) select).getTotalTokens() * 1.0 / findTimes);
		System.out.println("length : " + ((TfidfPref) select).getScanLength() * 1.0 / findTimes + " : "
				+ ((TfidfPref) select).getTotalLength() * 1.0 / findTimes + "  details : "
				+ ((TfidfPref) select).getLowerLength() * 1.0 / findTimes + " : "
				+ ((TfidfPref) select).getUpperLength() * 1.0 / findTimes);
